...
SolveResult solve(const clarabel::kktsystem::KKTSystem& system, const std::vector<double>& rhs, const clarabel::core::Settings&) {
    ...
    SolveResult res;
    const std::size_t primal_dim = system.n_primal;
    const std::size_t dual_dim = system.n_dual;
    res.delta_x.assign(solution.begin(), solution.begin() + primal_dim);
    res.delta_y.assign(solution.begin() + primal_dim, solution.begin() + primal_dim + dual_dim);
    return res;
}
